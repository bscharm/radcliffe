...
import (
	...
	"github.com/bscharm/radcliffe/radcliffe/parser"
...
func RadcliffeHandler(w http.ResponseWriter, r *http.Request) {
	...
	var response []radcliffe.Metadata
	m := make(chan radcliffe.Metadata)
	...
	parser.Parse(&wg, data, m)
	wg.Wait()
	respond(w, response)
}

func buildResponse(response *[]radcliffe.Metadata, m chan radcliffe.Metadata) {
	for md := range m {
...
func respond(w http.ResponseWriter, response []radcliffe.Metadata) {
